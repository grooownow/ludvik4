...
> Sub (muted, ~600px): «Сайты, веб-приложения и SaaS, плагины, автоматизация рутины — доводим до готового продукта.»
...
### 3. Услуги — «Что делаем» (сетка карточек)

> Section eyebrow «Что делаем». Grid of 4 cards (2×2 desktop, 1 col mobile), equal height,
...
> Section eyebrow «Как это работает». Lead line (medium): «Вы приходите с идеей или болью — предлагаем, как её решить.»
> Then 3 steps in a row (stack on mobile), numbered 01/02/03 (mono numerals):
> 01 **Поможем оформить задачу.** Расскажете, что хочется или какая задача стоит, — предложим варианты и придумаем решение под вас. Задачу и результат опишем вместе.
> 02 **Оптимальный MVP.** Рабочая версия в короткий срок, которая снимает острую боль.
> 03 **Формат под вас.** Разовый продукт, развитие или поддержка с доработками. Под смежные задачи — дизайн, бренд, видео, анимацию — подключаем нужных специалистов.

### 6. Кто за этим стоит

> Narrow, understated block (~640px), left-aligned. Small eyebrow «Кто за этим стоит».
> Text: «Ludvik4 — небольшая команда: разработчики, дизайнеры, ML-инженер. За плечами — годы в вебе, от продуктов в крупных компаниях до собственных приложений. Собираем цифровые продукты под ключ, работаем быстро и основательно, а под каждую задачу подключаем нужных людей.»
> No link, no photo, no name (team positioning; LinkedIn/personal links removed).

### 7. Контакт + форма

> Section eyebrow «Контакт». H2: «Расскажите, что нужно». Sub: «Опишите задачу — предложим решение и подскажем, с чего начать.»
...
> Minimal footer: «© 2026 Ludvik4» left; right — Telegram link only. Thin top border. Muted.
