...
| `docs/archive/`                       | Past one-off artifacts kept for history (e.g. the SEO audit that seeded the content plan) — nothing current lives here (see its README)                                                                  |
| `src/content/blog/`                   | Blog articles (`<slug>.mdx`) + each article's companion assets in `<slug>/` (cover source, off-site adaptations) — layout in `src/content/blog/README.md`, standards in `docs/rules/content.md`          |
| `docs/brand/`                         | Brand/social-profile assets not tied to one article (e.g. X avatar/cover)                                                                                                                                |
